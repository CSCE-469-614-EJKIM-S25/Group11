...
# Log file
LOG_FILE="projectrunscript.log"
HW4_SCRIPT="./projectrunscript" # Path to the script
...
for suite in "${!SUITES[@]}"; do
  for benchmark in ${SUITES[$suite]}; do
    for policy in "${REPLACEMENT_POLICIES[@]}"; do
      ...
      #                echo "$(date) - Skipping: ./projectrunscript $suite $benchmark $policy (Already executed)" | tee -a "$LOG_FILE"
      #                continue
      #            fi

      COMMAND="./projectrunscript $suite $benchmark $policy"
